...
class GaussianModel:
    ...
    def restore(self, model_args: GaussianModelCapture, training_args):
        ...
        self.optimizer.load_state_dict(model_args.optimizer_state)
    ...
    def load_ply(self, path, use_train_test_exp=False):
        ...
        if use_train_test_exp:
            ...
            if os.path.exists(exposure_file):
                ...
                print(f"No exposures to be loaded at {exposure_file}")
                self.pretrained_exposures = None
    ...
    def cat_tensors_to_optimizer(self, tensor_dict):
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
            assert len(group["params"]) == 1
            extension_tensor = tensor_dict[group["name"]]
            stored_state = self.optimizer.state.get(group["params"][0], None)
            if stored_state is not None:
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        selected_pts_mask = torch.logical_and(
            selected_pts_mask,
            torch.max(self.gert_scaling, dim=1).values
            <= self.percent_dense * scene_extent,
        )
